...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    // Look at the hashes to find the tokenAddress of payment? Decoding those will be more intensive than what I would think is the right move. Cause you would have to decode the hash, then go into its Object properties (of one param). Then you could start assessing the object properties for the token address of interest.
    //  - The problem with doing this is that you would have to access the onchain object database. Hmm. If I create it so that our subgraph listens for events emitting that new Orders have been created... then I have a database of order entities that I can pull from.
    // --->  These Order entities can be queried then based off of the details from the decoded Hash.
    // ---> So an Order comes through, it gets recorded. Then OrdersMatched() event is emitted, we decode the buy or sell hash, and we obtain the tokenAddress associated to it.
